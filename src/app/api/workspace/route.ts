...
export async function GET() {
  ...
  // Ensure tables exist in local D1 (handles Next.js miniflare isolated DBs)
  try {
    await db.batch([
      db.prepare(`
...
export async function POST(req: Request) {
  ...
  if (body.kind === 'folder') {
    ...
    return NextResponse.json({ id, name: name.trim(), sticker, color, user_id: user.id });
  }
  ...
  if (body.kind === 'file') {
    ...
    return NextResponse.json({ id, name: name.trim(), type, content, folder_id, user_id: user.id });
  }
